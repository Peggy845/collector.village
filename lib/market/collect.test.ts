...
describe('collectMarketRevenue', () => {
  ...
  it('已經入帳過的部分不會重複算，只算新賣出的差額', async () => {
    ...
    expect(fake.rows('market_furniture_slots')[0].collected_quantity).toBe(5);
  });
  ...
  it('併發搶輸（樂觀鎖 eq 條件不match）時不把這筆算進營收', async () => {
    ...
    fake.onceBeforeExecute('market_furniture_slots', 'update', () => {
      const row = fake.mutableRows('market_furniture_slots').find((r) => r.id === 14)!;
    ...
    expect(revenue).toBe(0);
    expect(fake.rows('market_furniture_slots')[0].collected_quantity).toBe(3);
  });
});
