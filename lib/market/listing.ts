...
// 把商品放上貨架的共用底層寫入邏輯（見 idea/排序太長.png 定案的優化，2026-08-02）：
// `/api/market/list`（手動上架單一品項、也是 ShelfCard「自動上架」按鈕依序呼叫的同一支API）
// 跟 `lib/market/restock.ts`（自動上架模式被動補貨）都呼叫這個函式，確保上架的寫入邏輯只有一份。
//
// 背景：自動上架模式每次補貨都是小量（例如貨架每分鐘賣掉1件、空出1件空位就補1件），
// 如果每次都新增一個 slot，同一款商品排隊中的 slot 會變成一長串「同款商品 × 1」的重複列，
// 排隊清單很快就長到看不下去。改成：檢查貨架目前排在最後面的那個 slot（不論正在賣還是排隊中）
// 是不是剛好跟這次要上架的品項（同格式+同設計圖）一樣，是的話直接把數量累加上去，不新增一列；
// 只有最後一個 slot 是不同品項（或貨架本來還沒有任何 slot）時，才真的新增一列接在後面。
// 因為只跟「最後一個」slot 合併，合併時只需要改它自己的 quantity（它的 active_from 不用動，
// 賣完時間自然因為 quantity 變大而順延），後面沒有其他 slot 需要跟著調整。
//
// 呼叫端負責先確認這次要上架的數量沒有超過貨架剩餘空位，這裡只負責實際寫入資料庫。
export async function upsertShelfSlot(
  admin: SupabaseClient,
  params: { shelfId: number; formatKey: string; designId: number; quantity: number; now: number }
): Promise<{ error?: string }> {
  const { shelfId, formatKey, designId, quantity, now } = params;
  ...
  // 找「結束時間最晚」的當成隊伍尾端。同一個貨架的排程理論上只會有單一一條隊伍，
  ...
  let tail: MarketShelfSlot | null = null;
  let latestFinish = 0;
  ...
  const { error } = await admin.from('market_shelf_slots').insert({
    shelf_id: shelfId,
