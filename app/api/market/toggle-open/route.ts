...
import { closeMarket, reopenMarket } from '@/lib/market/toggleOpen';
...
export async function POST() {
  ...
  const { data: userRow } = await admin.from('users').select('market_open').eq('id', user.id).maybeSingle();
  const isOpen = userRow?.market_open ?? true;

  if (isOpen) {
    const { error } = await closeMarket(admin, user.id);
    if (error) {
  ...
  const { error } = await reopenMarket(admin, user.id);
  if (error) {
