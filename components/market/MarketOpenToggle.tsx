...
export default function MarketOpenToggle({ open }: { open: boolean }) {
  ...
  async function handleToggle() {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/market/toggle-open', { method: 'POST' });
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      ...
      >
        {loading ? '切換中…' : open ? '營業中・點擊暫停營業' : '暫停營業中・點擊重新開店'}
      </button>
